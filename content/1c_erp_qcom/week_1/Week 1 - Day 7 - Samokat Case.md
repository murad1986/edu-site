...
```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: Временная шкала эволюции «Самоката» слева направо. 2018: один прямоугольник «1С:УТ» + иконка Excel. 2019: 30 разрозненных прямоугольников с подписью «30 баз». 2020: один большой прямоугольник «1C:ERP 2.5» с 30 точками-складами внутри. 2021: тот же прямоугольник с зелёными индикаторами у каждой точки. Над шкалой — график: линия «время закрытия месяца» падает от 21 до 5 дней.
Заголовок: «Эволюция архитектуры Самоката: 2018–2021»
```
...
> [!IMPORTANT] Главный урок
> Структура юрлиц — это фундамент, который невозможно менять «на ходу». Каждое лишнее юрлицо — это не просто строка в справочнике «Организации». Это постоянный «налог» на каждую операцию: документы, НДС, бухгалтеры, банковские комиссии, трансфертные цены. Проектируйте структуру минимально необходимой — как мы делали в [[Week 1 - Day 6 - Enterprise Design Workshop|Дне 6]].

```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: Сравнение «До» и «После». Левая половина: три прямоугольника (ООО Запад, ООО Восток, ООО Логистика), между ними двойные стрелки «Реализация/Приобретение» с красной подписью «+3% наценка, +НДС». Правая половина: один прямоугольник «ООО ГородЕда» с тремя зонами внутри (Хаб, Запад, Восток), между зонами простые стрелки «Перемещение». Над правой схемой зелёная подпись «−6,5 п.п. экономии».
Заголовок: «ГородЕда: цена лишних юрлиц»
```
...
Закрытие месяца в 1C:ERP 2.5 — это последовательность регламентных операций ([[Week 1 - Day 4 - Posting Logic|День 4]]). Каждая операция обрабатывает все документы за период. При линейном росте числа дарксторов объём документов рос **экспоненциально**:
...
```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: График с двумя осями. Ось X — число дарксторов (15, 30, 45). Ось Y — дни на закрытие месяца. Красная кривая показывает экспоненциальный рост (4 → 7 → 18). Зелёная пунктирная линия — целевой уровень «5 дней». Точка «Оптимизация» на 45 дарксторах, где красная линия резко падает до зелёной. Подписи трёх горлышек: «ФИФО», «Направления», «Непроведённые документы».
Заголовок: «Кривая смерти: почему закрытие месяца растёт экспоненциально»
```
...
```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: Воронка сопоставления НСИ. Сверху два прямоугольника: «ДелиВери: 3 500 SKU» и «ПродуктВезу: 2 800 SKU». Стрелки сходятся в блок «Сопоставление: 2 200 общих». Оттуда три ветки: «Автоматически (40%)» — зелёная стрелка вниз, «Вручную (57%)» — жёлтая стрелка вниз, «Ошибки (3%)» — красная стрелка вниз с иконкой предупреждения. У ошибок подпись: «+12% возвратов». Внизу итог: «4 месяца, 12 млн руб.».
Заголовок: «Анатомия слияния НСИ: воронка сопоставления»
```
...
1. **На уровне НСИ** ([[Week 1 - Day 1 - Виды учета и Архитектура|День 1]]): в карточке каждого скоропортящегося товара включили признак «Учёт по сериям» с обязательным полем «Дата окончания срока годности».

2. **На уровне склада** ([[Week 1 - Day 4 - Posting Logic|День 4]]): при приёмке товара кладовщик сканировал штрих-код, и ТСД запрашивал дату производства. Система автоматически рассчитывала дату окончания срока годности и присваивала партию. Товар размещался по принципу FEFO: партия с ближайшим сроком — ближе к зоне отбора.
...
Партионный учёт дал побочный эффект, который оказался ценнее прямой экономии. Теперь финансовый директор мог видеть в отчётах ([[Week 1 - Day 5 - Reporting Basics|День 5]]):
...
```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: Два стеллажа — «До FEFO» и «После FEFO». На левом стеллаже коробки расположены хаотично, некоторые помечены красным крестом (просрочка). На правом стеллаже — упорядоченные коробки с датами, ближайшие сроки — у края полки, стрелка указывает направление отбора. Внизу два числа: «Списания 5,8%» (красный) и «Списания 1,9%» (зелёный).
Заголовок: «FEFO: от хаоса к порядку на полке»
```
...
|:-----|:-----|:-------|:---------------|:-------------|
| 1 | Виды учёта и архитектура | [[Week 1 - Day 1 - Виды учета и Архитектура\|День 1]] | 3 вида учёта, 2 режима направлений | ERP — это три «правды» об одном бизнесе: управленческая, бухгалтерская, налоговая |
| 2 | Архитектура предприятия | [[Week 1 - Day 2 - Архитектура предприятия\|День 2]] | Организации, склады, подразделения | Структура юрлиц определяет стоимость каждой операции |
| 3 | Учётная политика | [[Week 1 - Day 3 - Учётная политика\|День 3]] | ФИФО vs средневзвешенная, FEFO, УСН/ОСНО | Учётная политика — правила игры, которые нельзя менять на ходу |
| 4 | Документооборот и регистры | [[Week 1 - Day 4 - Posting Logic\|День 4]] | 3 типа регистров, ордерная схема | Документ — обещание, регистр — реальность |
| 5 | Отчётность и аналитика | [[Week 1 - Day 5 - Reporting Basics\|День 5]] | 3 слоя отчётности, ловушки интерпретации | Отчёт без понимания структуры данных — опасное оружие |
| 6 | Практикум | [[Week 1 - Day 6 - Enterprise Design Workshop\|День 6]] | 4 задания, сквозная верификация | Архитектура — это не набор настроек, а система компромиссов |
...
| 0 | Турист | Начните с [[Week 1 - Day 1 - Виды учета и Архитектура\|Дня 1]] заново |
...
```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: Матрица 5×4. Строки — пять компаний (Самокат, ГородЕда, Маркет-Экспресс, ДелиВери, ФермаБокс). Столбцы — четыре паттерна (Долг структуры, Масштаб, Данные, Люди). В каждой ячейке — иконка: зелёная галочка (паттерн присутствует) или серый прочерк. Итоговая строка снизу показывает, сколько компаний из пяти затронул каждый паттерн. Подпись: «Все кейсы, все паттерны».
Заголовок: «Матрица паттернов: какая ошибка убивает чаще?»
```
