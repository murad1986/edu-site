...
Content: Карта города Новосибирска (схематично, без детальной карты — прямоугольники районов). 8 точек дарксторов отмечены синими маркерами, центральный хаб — крупным синим квадратом с подписью «Хаб». Стрелки от хаба к дарксторам показывают маршруты пополнения. Два даркстора помечены зелёной рамкой с подписью «ИП». Рядом легенда: «ООО — 6 точек, ИП — 2 точки».
Заголовок: «Физическая сеть НочьЭкспресс: 8 дарксторов + хаб»
```
...
```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: Mermaid-подобная схема из двух блоков. Левый блок «ООО НочьЭкспресс» содержит Хаб и 6 дарксторов — между ними простые стрелки «Перемещение товаров». Правый блок «ИП Карпов» содержит 2 даркстора. Между блоками — двойная стрелка, подписанная «Реализация → Приобретение (с НДС)». Красный восклицательный знак над двойной стрелкой.
Заголовок: «Два юрлица = два контура учёта»
```
...
В [[Week 1 - Day 3 - Учётная политика|Дне 3]] мы разбирали, как учётная политика определяет «правила игры» для всей системы. Теперь применим эти знания.
...
Но не для всех категорий. Бытовая химия (20% ассортимента) не требует учёта серий — это лишняя нагрузка на сканирование при приёмке и отгрузке. В [[Week 1 - Day 3 - Учётная политика|Дне 3]] мы называли это «налогом на процессор»: каждое дополнительное поле при сканировании увеличивает время обработки на 3–5 секунд.
...
В [[Week 1 - Day 1 - Виды учета и Архитектура|Дне 1]] мы говорили о двух режимах направлений деятельности. Для «НочьЭкспресс» с его 8 точками нужен **обособленный учёт** (Режим 2). Каждый даркстор — отдельное направление, со своими остатками и своим P&L.
...
```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: Таблица-матрица. Строки — 4 параметра учётной политики (Себестоимость, Налогообложение, Партии, Направления). Столбцы — ООО и ИП. В каждой ячейке — выбранное значение. Ячейки с различиями подсвечены оранжевой рамкой (себестоимость одинаковая — ФИФО, налогообложение разное — ОСНО vs УСН, партии одинаковые, направления одинаковые). Заголовок таблицы: «Сводная матрица учётной политики».
Заголовок: «Матрица учётной политики: ООО vs ИП»
```
...
В [[Week 1 - Day 5 - Reporting Basics|Дне 5]] мы обсуждали ловушку «средней маржи» — теперь вы видите её в действии. Если оценивать прибыльность точки только по молоку, вывод будет пессимистичным. Если только по бытовой химии — оптимистичным. Истина — в совокупности, и именно для этого нужна правильно настроенная аналитика.

```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: Вертикальная цепочка из 4 блоков, соединённых стрелками вниз. Блок 1: «Закупка» — иконка коробки, подпись «+200 шт., 13 600 руб.». Блок 2: «Перемещение» — иконка грузовика, подпись «−50 шт. хаб → даркстор». Блок 3: «Продажа» — иконка корзины, подпись «−1 шт., выручка 109 руб.». Блок 4: «P&L» — иконка графика, подпись «Маржа 25,1%». Справа от каждого блока — список затронутых регистров.
Заголовок: «Путь литра молока: от поставщика до P&L»
```
...
```text
Style: Technical process diagram, Blueprint aesthetic, clean ink lines, white background, schema style, no people, corporate blue (#0055BB). Landscape 2:1, 1200×600 px. Весь текст на русском.
Content: Архитектурная схема в стиле чертежа здания. Фундамент подписан «Организации (2)» и «Склады (9)». Первый этаж — «Учётная политика: ФИФО, партии, FEFO». Второй этаж — «Направления деятельности (8) + Подразделения (4)». Крыша — «Управленческий P&L». На крыше три флажка: «По точкам», «По бизнес-модели», «По ЦФО». Рядом с фундаментом — трещина с подписью «Риск: два юрлица = консолидация».
Заголовок: «Архитектура ERP для НочьЭкспресс: от фундамента до крыши»
```
...
|:--|:--------------|:----------|:-------|
| 1 | Все юрлица заведены в справочник «Организации» | [[Week 1 - Day 2 - Архитектура предприятия\|День 2]] | ☐ |
| 2 | Учётная политика заполнена для каждой организации | [[Week 1 - Day 3 - Учётная политика\|День 3]] | ☐ |
| 3 | Метод себестоимости выбран и зафиксирован | [[Week 1 - Day 3 - Учётная политика\|День 3]] | ☐ |
| 4 | Склады созданы по физическим точкам, не по юрлицам | [[Week 1 - Day 2 - Архитектура предприятия\|День 2]] | ☐ |
| 5 | Ордерная схема включена на всех складах | [[Week 1 - Day 4 - Posting Logic\|День 4]] | ☐ |
| 6 | Адресное хранение настроено (хотя бы упрощённое) | [[Week 1 - Day 2 - Архитектура предприятия\|День 2]] | ☐ |
| 7 | Партионный учёт включён для скоропорта | [[Week 1 - Day 3 - Учётная политика\|День 3]] | ☐ |
| 8 | Направления деятельности отражают структуру бизнеса | [[Week 1 - Day 1 - Виды учета и Архитектура\|День 1]] | ☐ |
| 9 | Подразделения (ЦФО) заведены для аналитики расходов | [[Week 1 - Day 2 - Архитектура предприятия\|День 2]] | ☐ |
| 10 | Комиссионная торговля отделена от собственной | [[Week 1 - Day 5 - Reporting Basics\|День 5]] | ☐ |
| 11 | Тестовый документ проведён и проверены движения | [[Week 1 - Day 4 - Posting Logic\|День 4]] | ☐ |
| 12 | Управленческий отчёт построен и верифицирован | [[Week 1 - Day 5 - Reporting Basics\|День 5]] | ☐ |
...
← [[Week 1 - Day 5 - Отчётность и аналитика|День 5]] | [[README|Оглавление]] | [[Week 1 - Day 7 - История Самоката|День 7]] →
