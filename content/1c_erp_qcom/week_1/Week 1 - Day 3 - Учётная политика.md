...
```mermaid
flowchart LR
    subgraph M1["Средняя за месяц"]
        direction TB
        M1A["Считается: при закрытии месяца"]
        M1B["Партионный учёт: нет"]
        M1C["Точность Soft Close: низкая"]
        M1D["Скорость закрытия: высокая"]
        M1E["Итог: 6 305 руб. / 130 шт."]
    end
    subgraph M2["Среднескользящая"]
        direction TB
        M2A["Считается: при каждой операции"]
        M2B["Партионный учёт: нет"]
        M2C["Точность Soft Close: высокая"]
        M2D["Скорость закрытия: очень высокая"]
        M2E["Итог: 6 223 руб. / 130 шт."]
    end
    subgraph M3["ФИФО"]
        direction TB
        M3A["Считается: при закрытии месяца"]
        M3B["Партионный учёт: да"]
        M3C["Точность Soft Close: низкая"]
        M3D["Скорость закрытия: низкая"]
        M3E["Итог: 6 060 руб. / 130 шт."]
    end

    style M1 fill:#004488,color:#fff
    style M2 fill:#005599,color:#fff
    style M3 fill:#0077AA,color:#fff
```
