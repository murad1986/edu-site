...
sequenceDiagram
    ...
    participant WH as Склад (кладовщик/сборщик)
    participant Fin as Финансы (бухгалтер)

    App->>ERP: Заказ клиента (резервирование)
    Note over ERP: Регистр «Заказы клиентов»\nОстатки заблокированы
    ERP->>WH: Задание на сборку
    WH->>ERP: Реализация товаров (отгрузка)
    Note over ERP: Регистр «Товары на складах»\nОстатки уменьшены
    App->>ERP: Оплата онлайн
    ERP->>Fin: Поступление безналичных ДС
    Note over Fin: Регистр «Расчёты с клиентами»\nДебиторка закрыта
    Fin->>ERP: Закрытие месяца
    Note over ERP: Себестоимость рассчитана\nПроводки сформированы
```
