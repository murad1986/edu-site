...
```mermaid
flowchart LR
    EVT["Физический факт\nПриёмка 200 кг бананов"] --> OU["Оперативный учёт\n— скорость: &lt;1 сек\n— вопрос: сколько есть?\n— пользователь: кладовщик"]
    EVT --> UU["Управленческий учёт\n— скорость: часы\n— вопрос: зарабатываем?\n— пользователь: финдир"]
    EVT --> BU["Регламентированный учёт\n— скорость: дни–месяц\n— вопрос: юридически верно?\n— пользователь: бухгалтер"]

    style EVT fill:#0055BB,color:#fff
    style OU fill:#005500,color:#fff
    style UU fill:#0077CC,color:#fff
    style BU fill:#885500,color:#fff
```
...
flowchart LR
    ...
    style W1 fill:#0055BB,color:#fff
    style W2 fill:#0055BB,color:#fff
    style W3 fill:#0055BB,color:#fff
    style W4 fill:#0055BB,color:#fff
    style HC fill:#CC4400,color:#fff
    style SC fill:#993300,color:#fff
    style GB fill:#660000,color:#fff
```
